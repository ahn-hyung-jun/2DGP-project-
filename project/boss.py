...
class Boss_right_arm:
    ...
    def update(self):
        ...
        if self.fire_time%300 == 0 and self.HP > 0:
            self.right_arm_state = random.randint(1,6)
        elif self.HP < 0:
        ...
        if self.right_arm_state == 1:
            ...
            boss_pattern.pattern_1(self.fire_time, self.right_arm_x + 40, self.right_arm_y - 10, 1)
            for hero_bullet in game_world.get_objects(2):
        ...
        if self.right_arm_state == 2:
            self.right_arm_x, self.right_arm_y = self.body_x + 120, self.body_y - 80
            boss_pattern.pattern_2(self.fire_time, self.right_arm_x + 45, self.right_arm_y + 5, 1)
            for hero_bullet in game_world.get_objects(2):
...
class Boss_left_arm:
    ...
    def update(self):
        self.fire_time = self.fire_time + self.fire_speed
        if self.fire_time % 300 == 0 and self.HP > 0:
            self.left_arm_state = random.randint(1, 6)
        elif self.HP < 0:
        ...
        if self.left_arm_state == 1:
            self.left_arm_x, self.left_arm_y = self.body_x - 120, self.body_y - 80
            boss_pattern.pattern_1(self.fire_time, self.left_arm_x + 40, self.left_arm_y - 10, 0)
            for hero_bullet in game_world.get_objects(2):
                if get_distance(hero_bullet.x, hero_bullet.y, self.left_arm_x, self.left_arm_y + 60) < (
                        PIXEL_PER_METER * 2) ** 2:
                    # 라이플은 단순한 데미지
                    if hero_bullet.state == 1 or hero_bullet.state == 2:
                        self.left_arm_HP -= hero_bullet.damage
        ...
        if self.left_arm_state == 2:
            self.left_arm_x, self.left_arm_y = self.body_x - 120, self.body_y - 80
            boss_pattern.pattern_2(self.fire_time, self.left_arm_x + 45, self.left_arm_y + 5, 0)
            for hero_bullet in game_world.get_objects(2):
        ...
        if self.left_arm_state == 3:
            self.left_arm_x, self.left_arm_y = self.body_x - 120, self.body_y - 80
            boss_pattern.pattern_3(self.fire_time, self.left_arm_x + 20, self.left_arm_y - 60, 0)
            for hero_bullet in game_world.get_objects(2):
        ...
        if self.left_arm_state == 4:
            ...
            boss_pattern.pattern_4(self.fire_time, self.left_arm_x + 0, self.left_arm_y - 10, 0, self.degree)
            for hero_bullet in game_world.get_objects(2):
        ...
        if self.left_arm_state == 5:
            ...
            boss_pattern.pattern_5(self.fire_time, self.left_arm_x + 10, self.left_arm_y - 70, 0)
            for hero_bullet in game_world.get_objects(2):
                if get_distance(hero_bullet.x, hero_bullet.y, self.left_arm_x, self.left_arm_y + 40) < (PIXEL_PER_METER * 2) ** 2\
        ...
        if self.left_arm_state == 6:
            self.left_arm_x, self.left_arm_y = self.body_x - 70, self.body_y - 80
            boss_pattern.pattern_6(self.fire_time, self.left_arm_x + 20, self.left_arm_y - 15, 0)
            for hero_bullet in game_world.get_objects(2):
                if get_distance(hero_bullet.x, hero_bullet.y, self.left_arm_x, self.left_arm_y + 40) < (PIXEL_PER_METER * 2) ** 2:
                    # 라이플은 단순한 데미지
                    if hero_bullet.state == 1 or hero_bullet.state == 2:
